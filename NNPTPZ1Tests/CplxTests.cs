...
namespace NNPTPZ1.Mathematics.Tests
{
    ...
    public class CplxTests
    {
        ...
        public void AddTestPolynome()
        {
            ...
            poly.Coefficients.Add(new ComplexNumber() { RealPart = 1, ImaginariPart = 0 });
            ComplexNumber result = poly.Evaluate(new ComplexNumber() { RealPart = 0, ImaginariPart = 0 });
            var expected = new ComplexNumber() { RealPart = 1, ImaginariPart = 0 };
            Assert.AreEqual(expected, result);
            result = poly.Evaluate(new ComplexNumber() { RealPart = 1, ImaginariPart = 0 });
            expected = new ComplexNumber() { RealPart = 2, ImaginariPart = 0 };
            Assert.AreEqual(expected, result);
            result = poly.Evaluate(new ComplexNumber() { RealPart = 2, ImaginariPart = 0 });
